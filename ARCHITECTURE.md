...
- `external` datasources, polling first; `datasource_cache`; lazy activation verified against a mock server's request log.
- Sandbox iframe + Worker runner; transformer editor, run console, version history. Credentials stored as plaintext JSON in `secrets_ciphertext` with the Settings disclosure from §4.9 (encrypted in place on account creation, Phase 3).
- **Acceptance:** add a public REST API, reshape it with a transformer, chart it; hiding the chart stops polling within one interval; a transformer that calls `fetch` fails with a CSP error; an infinite loop is killed at `timeout_ms`.
